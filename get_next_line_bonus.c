...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/04 14:17:04 by fatturan          #+#    #+#             */
/*   Updated: 2023/08/04 15:01:43 by fatturan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	dest[fd] = ft_until_newline(fd, dest[fd]);
	if (!dest[fd])
		return (0);
	line = ft_cut_to_newline(dest[fd]);
	dest[fd] = ft_next_line(dest[fd]);
	return (line);
}
